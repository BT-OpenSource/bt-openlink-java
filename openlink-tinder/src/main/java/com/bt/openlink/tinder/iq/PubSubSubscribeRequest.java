...
public class PubSubSubscribeRequest extends OpenlinkIQ {
    ...
    private PubSubSubscribeRequest(@Nonnull Builder builder, @Nullable List<String> parseErrors) {
        ...
        getPubSubNodeId().ifPresent(nodeId -> subscribeElement.addAttribute("node", nodeId.value()));
        getJID().ifPresent(subscriber -> subscribeElement.addAttribute("jid", subscriber.toString()));
    }
    ...
    public static PubSubSubscribeRequest from(@Nonnull IQ iq) {
        ...
        if (subscribeElement != null) {
            PubSubNodeId.from(TinderPacketUtil.getNullableStringAttribute(subscribeElement, "node")).ifPresent(builder::setPubSubNodeId);
            TinderPacketUtil.getJID(TinderPacketUtil.getNullableStringAttribute(subscribeElement, "jid")).ifPresent(builder::setJID);
        }
